...
describe("Contract: Customer", function(){
	it("can get an orders price", async function(){
		return menuInstance.methods.getOrderPrice([0,5,1,5],1).call().then((res)=>{
...
describe("Order Process", function(){
	...
	it("Can access the items ordered from the restaurant account", function(){
		return orderInstance.methods.getItemRaw(0).call({from: theAccounts[2]}).then(function(item){
	...
	it("can be accepted by the restaurant", function(){
		return orderInstance.methods.setOrderStatus(1).send({from: theAccounts[2]}).then(function(){
			return orderInstance.methods.restaurantStatus().call();
		}).then(function(state){
			assert.equal(state,1);
		});
	...
	it("Can access the items ordered from the rider account", function(){
		return orderInstance.methods.getItemRaw(0).call({from: theAccounts[4]}).then(function(item){
	...
	it("Restaurant can signal prepairing if invoked from the owners account",function(){
		return orderInstance.methods.setOrderStatus(2).send({from:theAccounts[2],gas:3000000}).then(function(){
			return orderInstance.methods.restaurantStatus().call();
