...
function autocompleteSetArray(arrr){
	arr = arrr;
}

function autocompleteAddToArray(item){
	arr.push(item);
}

function autocomplete(inp, arrr, onNewEntryCallback) {
      ...
      if (e.keyCode == 40) {
        ...
        if (currentFocus > -1) {
          ...
          if (x) x[currentFocus].click();
        	...
        	onNewEntryCallback(this.value, inp.id.substr(8));
        }
      } else {
